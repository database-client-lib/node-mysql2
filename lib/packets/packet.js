    ...
    const parseGeometry=()=> {
      ...
      switch (wkbType) {
        ...
        case 7: // WKBGeometryCollection
          ...
          const items = [];
          for (i = num; i > 0; i--) {
            items.push(parseGeometry());
          }
          const type = this.getGeometryType(wkbType)
          switch (wkbType) {
            case 4: return `MULTIPOINT(${items.map(item => item.replace(/.+?\((.+?)\)/, '$1')).join(', ')})`
            case 7: return `${type}(${items.join(', ')})`
            default:
              return `${type}(${items.map(item => item.replace(/\w+ *\(/, '(')).join(', ')})`
          }
      }
    }
